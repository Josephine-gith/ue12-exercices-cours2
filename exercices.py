...
# %%
def square(arr):
    return arr**2  # 👈 Insérez le code ici
...
def sin_values():
    ...
    return np.sin(th)  # 👈 Insérez le code
...
def g_vectorized(x):
    return np.where(x<=0,x,x**2)  # 👈 Insérez le code ici
...
# %%
def select_even(arr):
    return np.where(arr%2==0,arr,arr)  # 👈 Insérez le code ici
